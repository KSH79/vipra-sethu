...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename  = 'daily_shlokas'
      and policyname = 'daily_shlokas_select'
  ) then
    create policy daily_shlokas_select
      on public.daily_shlokas
      for select
      using (true);
  end if;
end
...
grant select on public.daily_shlokas to anon, authenticated;
