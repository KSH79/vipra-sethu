...
export async function POST(req: NextRequest) {
  try {
    ...
    const badge_color = body.badge_color?.trim() || null
    ...
    const { data, error } = await supabase
      .from(TABLE)
      .insert({ code, name, description, icon, badge_color, display_order, is_active, created_by: userId, updated_by: userId })
      .select('*')
      .single()
    ...
    return NextResponse.json({ ok: false, error: e?.message || 'Failed to create category' }, { status: 400 })
  }
}
