...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      .from(TABLE)
      .insert({ code, name, description, display_order, is_active, created_by: userId, updated_by: userId })
      .select('*')
      .single()
    ...
    return NextResponse.json({ ok: false, error: e?.message || 'Failed to create sampradaya' }, { status: 400 })
  }
}
