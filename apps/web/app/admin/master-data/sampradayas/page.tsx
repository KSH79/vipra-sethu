...
export default function SampradayasManager() {
  ...
  const [form, setForm] = useState<Partial<Sampradaya>>({ code: "", name: "", description: "", display_order: 0, is_active: true });
  ...
  async function load() {
    try {
      ...
      const res = await fetch(`/api/admin/master-data/sampradayas?${params.toString()}`, { cache: "no-store" });
  ...
  async function onSave() {
    try {
      ...
      const url = isEdit ? `/api/admin/master-data/sampradayas/${encodeURIComponent(editing!.code)}` : "/api/admin/master-data/sampradayas";
  ...
  async function onDelete(code: string) {
    if (!confirm(`Soft delete sampradaya ${code}?`)) return;
    const res = await fetch(`/api/admin/master-data/sampradayas/${encodeURIComponent(code)}`, { method: "DELETE" });
    const json = await res.json();
    if (!json.ok) return alert(json.error || "Delete failed");
    await load();
  }

  async function onToggleActive(row: Sampradaya) {
    const res = await fetch(`/api/admin/master-data/sampradayas/${encodeURIComponent(row.code)}`, {
      method: "PATCH",
  ...
  function openEdit(row: Sampradaya) {
    setEditing(row);
    setForm({
      code: row.code,
      name: row.name,
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Sampradayas</h1>
          <p className="mt-1 text-slate-600">Manage religious traditions used across the platform.</p>
        </div>
        <button onClick={openCreate} className="rounded-md bg-saffron-700 px-3 py-2 text-white text-sm font-medium hover:bg-saffron-800">Add Sampradaya</button>
      </div>
      ...
      <div className="mt-6 overflow-x-auto rounded-lg border border-slate-200 bg-white">
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>
            ...
            {!loading && !error && filtered.length === 0 && (<tr><td colSpan={5} className="px-4 py-8 text-center text-slate-500">No sampradayas found</td></tr>)}
            {filtered.map((row) => (
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-lg">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">{editing ? 'Edit Sampradaya' : 'Add Sampradaya'}</h2>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4">
              {!editing && (
                <div>
                  ...
                  <input value={String(form.code || '')} onChange={(e)=>setForm(f=>({...f, code:e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-saffron-500" placeholder="e.g., madhwa" />
                </div>
              )}
              <div>
                <label className="block text-sm text-slate-700">Name</label>
                <input value={String(form.name || '')} onChange={(e)=>setForm(f=>({...f, name:e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-saffron-500" placeholder="e.g., Madhwa" />
              </div>
              <div>
