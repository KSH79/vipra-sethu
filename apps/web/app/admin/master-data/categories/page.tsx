...
export default function CategoriesManager() {
  ...
  const [form, setForm] = useState<Partial<Category>>({ code: "", name: "", description: "", icon: "", badge_color: "", display_order: 0, is_active: true });
  ...
  async function onSave() {
    try {
      ...
      setShowForm(false);
      setEditing(null);
      setForm({ code: "", name: "", description: "", icon: "", badge_color: "", display_order: 0, is_active: true });
      await load();
    } catch (e: any) {
  ...
  async function onDelete(code: string) {
    if (!confirm(`Soft delete category ${code}?`)) return;
  ...
  async function onToggleActive(row: Category) {
    ...
    <div className="mx-auto max-w-6xl px-6 py-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-600">Manage service categories used across the platform.</p>
        </div>
        <button onClick={openCreate} className="rounded-md bg-saffron-700 px-3 py-2 text-white text-sm font-medium hover:bg-saffron-800">Add Category</button>
      </div>

      {/* Filters */}
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="Search by code/name/description" className="w-64 rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-saffron-500" />
      ...
      <div className="mt-6 overflow-x-auto rounded-lg border border-slate-200 bg-white">
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>
            ...
            {error && !loading && (<tr><td colSpan={7} className="px-4 py-8 text-center text-red-600">{error}</td></tr>)}
            {!loading && !error && filtered.length === 0 && (<tr><td colSpan={7} className="px-4 py-8 text-center text-slate-500">No categories found</td></tr>)}
            {filtered.map((row) => (
              <tr key={row.code} className="border-t border-slate-100">
                <td className="px-4 py-2 font-mono text-xs text-slate-700">{row.code}</td>
                <td className="px-4 py-2">{row.name}</td>
                <td className="px-4 py-2 text-slate-600">{row.icon || '—'}</td>
                <td className="px-4 py-2 text-slate-600">{row.badge_color || '—'}</td>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-lg">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">{editing ? 'Edit Category' : 'Add Category'}</h2>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4">
              {!editing && (
                <div>
                  ...
                  <input value={String(form.code || '')} onChange={(e)=>setForm(f=>({...f, code:e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-saffron-500" placeholder="e.g., purohit" />
                </div>
              )}
              <div>
                <label className="block text-sm text-slate-700">Name</label>
                <input value={String(form.name || '')} onChange={(e)=>setForm(f=>({...f, name:e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-saffron-500" placeholder="e.g., Vedic Purohit" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-slate-700">Icon</label>
                  <input value={String(form.icon || '')} onChange={(e)=>setForm(f=>({...f, icon:e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-saffron-500" placeholder="e.g., lucide:book" />
                </div>
                <div>
                  <label className="block text-sm text-slate-700">Badge Color</label>
                  <input value={String(form.badge_color || '')} onChange={(e)=>setForm(f=>({...f, badge_color:e.target.value}))} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-saffron-500" placeholder="e.g., #f59e0b or slate-600" />
                </div>
              </div>
